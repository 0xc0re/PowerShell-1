...
.DESCRIPTION
	This Powershell cmdlet loops through all Exchange 2007 or newer mailboxes and returns Exchange folder permissions (Inbox,Calendar,etc.), 
    Exchange mailbox permissions (FullAccess) and AD permissions (Send-As). Group membership is expanded for direct user to delegate relationships.
    ...
    NOTE: This script introduces resource load to the environment and should be executed off-hours. On average, processing time is around 30 - 45 minutes per 1000 mailboxes.
    This script must be executed from the Exchange Management Shell (EMS) on an administrative workstation or Exchange server.
    
	Developed by Brian Cheatham - brian@getcloudsavvy.com

.EXAMPLE
	Get-AllExchPermissions.ps1
.EXAMPLE
	Get-AllExchPermissions.ps1 -SkipFolderPermissions
.EXAMPLE
	Get-AllExchPermissions.ps1 -ExcludeAccount DOMAIN\Username
.EXAMPLE
	Get-AllExchPermissions.ps1 -CSVFile C:\Reports\AllExchPermisssions.csv
#>
